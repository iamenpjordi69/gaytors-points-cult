...
class addscores_admin(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
    
    @app_commands.command(name="addscores_admin", description="Add points to a user (Manage Server required)")
    @app_commands.describe(user="User to add points to", points="Points to add")
    async def addscores_admin(self, interaction: discord.Interaction, user: discord.Member, points: float):
        try:
            # Permission check
            ...
            print(f"Error in addscores_admin command: {e}")
            import traceback
            traceback.print_exc()

async def setup(bot):
    await bot.add_cog(addscores_admin(bot))
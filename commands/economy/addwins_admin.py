...
class addwins_admin(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
    
    @app_commands.command(name="addwins_admin", description="Add wins to a user (Manage Server required)")
    @app_commands.describe(user="User to add wins to", wins="Wins to add")
    async def addwins_admin(self, interaction: discord.Interaction, user: discord.Member, wins: int):
        try:
            # Permission check
            ...
            print(f"Error in addwins_admin command: {e}")
            import traceback
            traceback.print_exc()

async def setup(bot):
    await bot.add_cog(addwins_admin(bot))
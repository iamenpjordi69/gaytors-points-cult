...
class removescores_admin(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
    
    @app_commands.command(name="removescores_admin", description="Remove points from a user (Manage Server required)")
    @app_commands.describe(user="User to remove points from", points="Points to remove")
    async def removescores_admin(self, interaction: discord.Interaction, user: discord.Member, points: float):
        try:
            # Permission check
            ...
            print(f"Error in removescores_admin command: {e}")

async def setup(bot):
    await bot.add_cog(removescores_admin(bot))
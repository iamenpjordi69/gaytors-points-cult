...
class removewins_admin(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
    
    @app_commands.command(name="removewins_admin", description="Remove wins from a user (Manage Server required)")
    @app_commands.describe(user="User to remove wins from", wins="Wins to remove")
    async def removewins_admin(self, interaction: discord.Interaction, user: discord.Member, wins: int):
        try:
            # Permission check
            ...
            print(f"Error in removewins_admin command: {e}")

async def setup(bot):
    await bot.add_cog(removewins_admin(bot))